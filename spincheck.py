...
import cfenv
from flight import MAX_THRUST, Keyboard, clamp
...
def run(motor: int = 1, uri: str | None = None) -> None:
    ...
    with cfenv.connect(uri) as scf:
        ...
        if cf.param.values.get("system", {}).get("selftestPassed") == "0":
            # The firmware tests MPU6050, then HMC5883L, then MS5611, and stops
            # at the first failure, so the first 0 is the one that matters.
        ...
        power = 0
        selected = (motor,)                     # (1, 2, 3, 4) means all together
        marks: dict[str, list[str]] = {}
        vbat = None

        def label() -> str:
            return "all" if len(selected) == 4 else f"m{selected[0]}"

        def set_power(value: int) -> None:
            nonlocal power
            power = int(clamp(value, 0, MAX_THRUST))
            for m in selected:
                cf.param.set_value(f"motorPowerSet.m{m}", str(power))
                ...
                while True:
                    for key in kb.poll():
                        ...
                        if key == " ":
                            ...
                            set_power(0)
                            selected = (1, 2, 3, 4) if key == "0" else (int(key),)
                        elif key == "m":
                            marks.setdefault(label(), []).append(f"starts at {power}")
                        elif key == "n":
                            marks.setdefault(label(), []).append(f"stops at {power}")
                        elif key in "wsda":
                            # One step per keypress; holding the key repeats it.
                            set_power(power + {"w": BIG_STEP, "s": -BIG_STEP,
                                               "d": SMALL_STEP, "a": -SMALL_STEP}[key])
                    if battery:
                        vbat = battery[-1]["pm.vbat"]
                    bat = f"{vbat:.2f}V" if vbat is not None else "?    "
                    print(f"\r  {label():<4} power {power:>6}   bat {bat}   "
                          f"marks: {', '.join(marks.get(label(), [])) or '-':<40}",
                          end="", flush=True)
                    time.sleep(DT)
        ...
        print("\n\nMotors stopped.")
        for which, notes in marks.items():
